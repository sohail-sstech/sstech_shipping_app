...
class LabelController extends Controller
{
	...
	public function index()
    {
        ...
        return view('admin/label/view',array('store_details' => $storedetails));
    }	
	public function preload_labellist(Request $request)
	{
		...
		$label_list_arr = Label::select('label_details.*','us.name as storename')->join('users as us', 'label_details.user_id', '=','us.id')
			...
			})->where('is_manifested','like','%' .$_POST['is_manifest']. '%')->where('us.name','like', '%' .$store. '%')->take($_POST['length'])->skip(intval($_POST['start']))->orderBy('id', 'desc')->get()->toArray();
